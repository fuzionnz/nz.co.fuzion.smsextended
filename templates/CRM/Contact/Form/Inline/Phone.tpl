...
<table class="crm-inline-edit-form">
    ...
    <tr id="Phone_Block_{$blockId}" {if $blockId gt $actualBlockCount}class="hiddenElement"{/if}>
        ...
        <td>{$form.phone.$blockId.phone_type_id.html}<span id="is_sms_{$blockId}"><br />{$form.phone.$blockId.is_sms.html}{ts}Use for SMS{/ts}</span></td>
...
<script type="text/javascript">
  CRM.$(function($) {
    ...
    toggleSMS();
    $('select[id$="_phone_type_id"]').on('change', function() {
      toggleSMS();
    });
    ...
    function toggleSMS() {
      ...
      $.each(blocks, function(index, blockId) {
        if ($('#phone_' + blockId + '_phone_type_id').val() == 2) {
          $("#is_sms_" + blockId).show();
        }
        else {
          $("#is_sms_" + blockId).hide();
        }
      });
    }
